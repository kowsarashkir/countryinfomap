...
function getCountryHolidays(data, place) {
  console.log("HOLIDAY FUNCTION");
  console.log({ data });
    ...
    $.ajax({
      ...
      success: function (result) {
        ...
        if (result.data) {
          ...
          displayCountryHolidays(data, place);
          // displayCountryWeather(result.data, country);
        } else {
          console.log("Data object not found in the response.");
        }
...
function getCountryName(lat, lng) {
  console.log("You called dgetcountryNamwe");
  console.log({ lat, lng });
  return new Promise(function (resolve, reject) {
    $.ajax({
      url: "libs/php/countryInfo.php",
...
function selectCountry(val) {
  ...
  // let countriesData;
  let countriesData; // Assuming this is declared globally or in the appropriate scope

  fetchCountries().then((res) => {
    console.log("GOT HERE ");
    console.log({ res });

    // Set countriesData here
    countriesData = res;

    // Continue with any code that depends on countriesData
    ...
    countriesData.map(function (item) {
      ...
      // Call functions or perform actions that depend on countriesData
      // ...
    });
